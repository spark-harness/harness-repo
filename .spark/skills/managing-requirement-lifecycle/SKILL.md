...
- Gate JSON template: `context/harness-framework/templates/gate-report.gate.json`
...
6. For gate checks, write a fixed-format `*.gate.json` file under `requirements/{requirement-id}/gates/`.
7. Run `janus gate validate <gate-json>`.
8. Render audit Markdown with `janus gate render --input <gate-json> --output <gate-md>`.
9. Use stage advancement checks only when deciding whether the gate can release a stage.
